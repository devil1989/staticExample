...
const mutations={//state是store封装的时候默认传入的，args是通过store.commit('setName'，args)传入的
	...
	updateSafeInfo(state,args){
		state.safeInfo=Object.assign({},state.safeInfo,args);
	},
	updateSafeInfoPwd(state,args){
		state.safeInfo.data=Object.assign({},state.safeInfo.data,args);
	},

	//修改密码的返回结果
	updateSafeChangedResult(state,args){
		state.safeInfo.changeResult=Object.assign({},state.safeInfo.changeResult,args);
	},

	//更新个人中心的图片信息
	updateUserPic(state,args){
		state.userInfo.data.userPic=Object.assign({},state.userInfo.data.userPic,args);
	},

	//更新用户昵称
	updateUserNickname(state,args){
		state.userInfo.data.userName=Object.assign({},state.userInfo.data.userName,args);
	},

	updateUserPhone(state,args){
		state.userInfo.data.phone=Object.assign({},state.userInfo.data.phone,args);
	},

	updateUserName(state,args){
		state.userInfo.data.name=Object.assign({},state.userInfo.data.name,args);
	},

	updateUserAge(state,args){
		state.userInfo.data.age=Object.assign({},state.userInfo.data.age,args);
	},

	updateUserSex(state,args){
		state.userInfo.data.sex=Object.assign({},state.userInfo.data.sex,args);
	},
	...
	//更新个人中心的粉丝列表
	updateFansList(state,args){
		state.fansList=Object.assign({},state.fansList,args);
	},

	//更新个人中心的关注列表
	updateAttentionList(state,args){
		state.attentionList=Object.assign({},state.attentionList,args);
	},

	//更新人员信息列表
	updatePersonList(state,args){
		state.personList=Object.assign({},state.personList,args);
	}
